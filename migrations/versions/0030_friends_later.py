...
mixed into the «add me» friend-link HMAC. The first successful add via a link bumps it, making the link
SINGLE-USE — once someone adds you it (and any copies) stop working, so the link can't be broadcast and
needs no manual reset. It never touches anyone else's link or any event-invite sig (those share one global
secret). ver=0 keeps the legacy payload, so every link already minted stays valid until first used.
"""
