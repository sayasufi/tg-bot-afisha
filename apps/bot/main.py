...
COMMANDS = [
    ...
    BotCommand(command="suggest", description="Предложить событие или канал"),
]

# Канон позиционирования: «внезапно свободный вечер» (job = спонтанность), не «афиша» (там четыре
# миллиардных бренда). Саппорт-столпы: эксклюзив из каналов площадок + бот сам напомнит.
DESCRIPTION = (
    "Свободный вечер? Окрест — карманная карта событий: открой и за десять секунд "
    "увидишь, куда успеваешь прямо сейчас рядом. Концерты, выставки, стендап, лекции — "
    "включая события из каналов самих площадок, которых нет на афишных сайтах. "
    "Сохрани — напомню за два часа до начала."
)
SHORT_DESCRIPTION = "Карта событий рядом: куда успеваешь прямо сейчас. Сохрани — напомню за 2 часа."
