...
LLM_REL = {
    "афиша": (1.0, "афиша"),
    "город": (0.60, "город/локалка"),  # новости/локалка вторичны для афиша-аппа (было 0.85 → топ забивали ЧП/новости)
    "тема": (0.80, "тема?"),
    "мусор": (0.10, "не тема"),
}
...
_PROMPT = (
    ...
    "Определи ГОРОД канала (каноническое русское имя: Москва, Санкт-Петербург, Казань, Нижний Новгород…). "
    "ВАЖНО: если канал о ЗАРУБЕЖНОЙ стране/городе (Беларусь, Казахстан, Украина, Армения, Грузия и т.п.) — "
    "верни этот город ИЛИ страну (Минск, Алматы, Киев, Беларусь): это критично для гео-фильтра. Если город "
    "не ясен из постов — пустая строка.\n"
    'Верни ТОЛЬКО JSON без пояснений: {"category":"афиша|город|тема|мусор","city":"<город/страна или пусто>"}\n'
    "Примеры:\n"
    ...
    'Вандруй — нескучно об истории мест Беларуси, экскурсии и события (@vandrui) → {"category":"афиша","city":"Беларусь"}\n'
    'Afisha Almaty — события Алматы, билеты (@almaty_afisha) → {"category":"афиша","city":"Алматы"}\n'
    'ND — Новости Дня, федеральные новости/криминал (@ndnews24) → {"category":"мусор","city":""}\n'
