...
def _relevance(title: str | None, username: str | None) -> tuple[float, str]:
    ...
    ×0.10). Только если афиша-ключа нет — режем мусор (×0.10). M7: город 0.60. C6a: «тема?» (не мусор, но
    и не подтверждённая афиша) снижена 0.80→0.65 — иначе неклассифицированный off-topic пролезал в вердикт
    «брать» (quality 90 × 0.80 = 72 ≥ 65); с ×0.65 такой канал упирается в «осторожно», пока LLM не подтвердит тему."""
