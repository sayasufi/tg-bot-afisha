...
function releasePage(theme, slot) {
  const next = waiters[theme].shift();
  if (next) next(slot); // передаём занятый слот следующему в очереди
...
server.listen(PORT, () => console.log(`tiles on :${PORT} → ${RENDER_BASE}, cache ${CACHE_DIR}`));
