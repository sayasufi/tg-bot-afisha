...
Отдаём ВСЕ каналы разом (~323) с именем города (джойн ref.cities) — фильтрация/сортировка на клиенте.
Добавление: case-insensitive upsert (LOWER-match → не дублит mixed-case) + COALESCE venue (re-add не
затирает привязку). Тумблер/привязка под require_admin, все SQL через text() c bind-параметрами.
"""
...
async def list_channel_cities(
    ...
    """Справочник городов (city_id → имя) для формы добавления."""
    rows = (await db.execute(text(
        "SELECT c.city_id, c.name, count(tc.channel_id) AS channels "
        "FROM ref.cities c JOIN ref.telegram_channels tc ON tc.city_id = c.city_id "
...
async def add_venue_channel(
    ...
    """Добавить/реактивировать канал. username нормализуется (case-insensitive upsert)."""
    ...
    # Case-insensitive: существующие username бывают в смешанном регистре (сид не лоуэркейзил),
    # а ON CONFLICT (username) регистрозависим → искали бы по точному и создали бы ДУБЛЬ.
    existing = (await db.execute(
        text("SELECT channel_id FROM ref.telegram_channels WHERE LOWER(username) = :u"), {"u": username}
    )).first()
    if existing:
        cid = existing[0]
        # COALESCE: re-add НЕ затирает существующую привязку, если её не передали (для смены — /bind).
...
async def toggle_venue_channel(
    ...
    """Включить/выключить канал (is_active). Фетч читает это поле live (следующий прогон)."""
    ...
    await db.commit()
    await write_audit(db, request, actor, "channel.toggle", target=str(channel_id), params={"active": active}, result="ok")
...
async def bind_venue_channel(
    ...
    """(Пере)привязка площадки к каналу: venue_name/venue_address (+ опц. city_id). Пустые → NULL (общий канал)."""
    venue_name = payload.get("venue_name")
    venue_address = payload.get("venue_address")
    if isinstance(venue_name, str):
        venue_name = venue_name.strip() or None
