...
Safe by design:
  * category is only overwritten when the LLM actually answered (not the
    offline "fallback"); if the model is unreachable, categories are left as-is.
...
from core.db.models import Event, EventCandidate, EventOccurrence, EventSource, RawEvent
...
def main() -> None:
    ...
    db = SessionLocal()
    llm = LLMService()
    stats = {"seen": 0, "cat_changed": 0, "cat_kept": 0, "priced": 0, "llm_down": 0}
    try:
        ...
        for e in events:
            ...
            if src:
                ...
                if raw and isinstance(raw.raw_payload_json, dict):
                    payload = raw.raw_payload_json
                    hints = _source_hints(payload)

            # 1) Re-classify with source hints.
