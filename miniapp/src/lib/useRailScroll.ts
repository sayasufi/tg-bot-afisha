...
// finger; here the content tracks at `factor` (<1), so you move your finger a little farther
// for the same scroll — a calmer rail. A light inertia on release keeps it gliding (not a
// dead stop). Vertical drags fall through untouched, so the page still scrolls normally.
export function useRailScroll<T extends HTMLElement = HTMLDivElement>(factor = 0.85) {
