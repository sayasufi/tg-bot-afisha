...
export function FocusBar({ event, onOpen, onClose }: { event: EventItem; onOpen: (i: EventItem) => void; onClose: () => void }) {
  const meta = categoryMeta(event.category);
  const when = formatWhenShort(event.date_start, event.date_end);
  return (
    <div className="focusbar" style={{ "--cat": meta.color } as CSSProperties}>
