...
// Shared between the live vector basemap (basemap.tsx) and the server-side tile
// renderer (tilerender.ts) — ONE source of truth for how the map looks.
...
export const PALETTE: Record<ThemeName, MapPalette> = {
  light: {
    bg: "#f4f4ef",
    water: "#cddfeb",
    waterway: "#a3c3db",
...
export const STYLE_URL: Record<ThemeName, string> = {
  light: "https://tiles.openfreemap.org/styles/positron",
  dark: "https://tiles.openfreemap.org/styles/dark",
};

// Re-skins a loaded maplibre style into the gallery look: palette repaints, Cyrillic-only
// labels, and the layers the stock style omits (house numbers, POIs, metro, parks, far-zoom
// atlas texture). Pure style work only — no Leaflet, no event handlers — so the server-side
// tile renderer can call it too. `placesBase` prefixes the /v1/places sources (the renderer
// runs on a different origin than the API).
export function tuneMaplibreStyle(mlMap: any, theme: ThemeName, opts?: { placesBase?: string }): void {
  const pal = PALETTE[theme];
  const placesBase = opts?.placesBase ?? "";
  ...
  // Underground rivers (Напрудная, Неглинная… — brunnel=tunnel/culvert collectors) must not
  // draw lines or names across city blocks. Excluding on brunnel is safe: features without
  // the property keep matching ("in" → false → "!" → true).
  const notUnderground = ["!", ["in", ["get", "brunnel"], ["literal", ["tunnel", "culvert"]]]];
  for (const layer of layers) {
    const sl = layer["source-layer"];
    if (sl !== "waterway" && sl !== "water_name") continue;
    try {
      const cur = mlMap.getFilter(layer.id);
      mlMap.setFilter(layer.id, cur ? ["all", cur, notUnderground] : notUnderground);
    } catch {
      /* filter rejects the merge — leave as-is */
    }
  }
  for (const layer of layers) {
  ...
  addLayer(
    {
      ...
      filter: ["all", ["==", ["get", "class"], "river"], notUnderground],
      maxzoom: 12,
      paint: {
  ...
  try {
    if (!mlMap.getSource("parks")) {
      mlMap.addSource("parks", { type: "geojson", data: `${placesBase}/v1/places?kind=park&city=Moscow` });
    }
  } catch {
    /* ignore */
  }
  ...
  try {
    if (!mlMap.getSource("metro")) {
      mlMap.addSource("metro", { type: "geojson", data: `${placesBase}/v1/places?kind=metro&city=Moscow` });
    }
  } catch {
    /* ignore */
  }
