...
class LLMExtractionService:
    ...
    async def extract_events_with_reason(
        ...
        prompt = (
            ...
            "Если пост — РОЗЫГРЫШ/конкурс билетов («дарим N билетов», «разыгрываем билеты»): верни конкретное "
            "анонсируемое событие с ЕГО реальным названием и датой (НЕ «Розыгрыш билетов»); если конкретного "
            "события с датой в посте нет — is_event:false. "
            "price_text: цена билета или «от N ₽» если указана; «бесплатно» если вход свободный/бесплатный "
            "(«вход свободный», «бесплатно», «участие бесплатное»); иначе пусто. "
            "city_hint="
            f"{city_hint}."
        )
        if post_date:
            prompt += (
                ...
                "«до HH:MM»/«до утра»/«до 03:00» — это время ОКОНЧАНИЯ, НЕ начала: в date_start его НЕ ставь "
                "(можешь в date_end). Если есть ТОЛЬКО время закрытия без часа начала — верни дату без времени. "
                "Если в посте указаны И время дверей/открытия, И время выхода артиста («открытие 22:00, артист в "
                "23:59») — в date_start ставь время ОТКРЫТИЯ (когда можно прийти), не выход артиста."
            )
        if venue_hint:
            prompt += (
