...
class DummyYandexMaps:
    """Keyless Yandex Maps scraper stub — returns nothing so the chain falls through."""

    def __init__(self, result: tuple[float, float, str] | None = None):
        self.result = result
        self.calls: list[tuple[str, str | None]] = []

    async def geocode(self, query: str, city_hint: str | None = None):
        self.calls.append((query, city_hint))
        return self.result
...
async def test_fallback_to_nominatim() -> None:
    ...
    svc.yandex = yandex
    svc.yandex_maps = DummyYandexMaps()
...
async def test_venue_osm_first_uses_nominatim_and_caches() -> None:
    ...
    # Yandex API is tried first (returns None here), second call is served from cache.
