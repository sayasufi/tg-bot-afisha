...
def test_price_text_round_trips_through_normalizer():
    ...
    assert c._price_text({"price_min": 0, "price_max": 0}) == ""  # 0/0 is ambiguous → no price, NOT free
    assert c._price_text(None) == ""  # unknown registration → blank
    assert c._price_text({"is_registration_open": True}) == ""  # status-only reg, no price → unknown
    assert parse_price_field(c._price_text({"price_min": 0, "price_max": 700})) == (0.0, 700.0)  # «до 700»
